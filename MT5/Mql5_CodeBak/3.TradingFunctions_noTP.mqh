...
void ProcessTradingLogic(double totalScore)
{
   ...
   if(totalScore >= DynamicBuyThreshold && totalPositions == 0)
   {
      ...
      PrintFormat("Attempting to open Buy position. Score: %.2f | Lot Size: %.5f | Stop Loss: %.5f", totalScore, dynamicLotSize, stopLossPrice);

      if(trade.Buy(dynamicLotSize, _Symbol, 0, stopLossPrice, 0, "Buy")) // Set TP to 0
      {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(PositionSelectByTicket(ticket))
      {
         if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
         {
            ...
            {
               if(trade.PositionModify(ticket, newSL, 0)) // Set TP to 0
               {
