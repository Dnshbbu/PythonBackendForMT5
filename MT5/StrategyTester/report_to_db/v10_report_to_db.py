...
def parse_strategy_tester_report(html_file):
    ...
    # Define the list of fields that need to be split
    fields_to_split = {
        ...
        'AHPR',
        'GHPR',
        'Profit Trades (% of total)',      # Updated to include parentheses
        'Loss Trades (% of total)',        # Updated to include parentheses
        'Maximum consecutive wins ($)',     # Updated to include parentheses
        'Maximum consecutive losses ($)',   # Updated to include parentheses
        'Maximal consecutive profit (count)', # Updated to include parentheses
        'Maximal consecutive loss (count)',   # Updated to include parentheses
        'Short Trades (won %)',            # Newly added
        'Long Trades (won %)'              # Newly added
    }
...
def save_to_csv(data, filename):
    ...
    try:
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            # if filename.endswith('settings') or filename.endswith('results'):
            if 'settings' in filename:
                writer.writerow(['Label', 'Value'])
                writer.writerows(data)
            # elif filename.endswith('orders'):
...
def save_settings_and_results_to_csv(data, run_id):
    ...
    if data.get('Settings'):
        save_to_csv(data['Settings'], f'{run_id}_settings.csv')
    if data.get('Results'):
        save_to_csv(data['Results'], f'{run_id}_results.csv')
...
def save_orders_and_deals_to_csv(data, run_id):
    ...
    """
    if data.get('Orders'):
        save_to_csv(data['Orders'], f'{run_id}_orders.csv')
    if data.get('Deals'):
        save_to_csv(data['Deals'], f'{run_id}_deals.csv')
