...
def extract_key_value_pairs(cells):
    ...
    Splits pairs with labels and/or values containing parentheses into separate entries.
    ...
    while i < len(cells):
        ...
        if cell_text.endswith(':'):
            ...
            # Now, split label and value if label contains '('
            split_pairs = split_label_value(label, value)
            pairs.extend(split_pairs)
        i += 1
    return pairs

def split_label_value(label, value):
    """
    Splits labels and values containing parentheses into separate key-value pairs.
    ...
    """
    # Regex to match labels like "Short Trades (won %)"
    label_match = re.match(r'^(.*?)\s*\((.*?)\)$', label)
    if label_match:
        main_label = label_match.group(1).strip()
        sub_label = label_match.group(2).strip()

        # Check if sub_label ends with '%'
        if sub_label.endswith('%'):
            # Remove '%' for the first label
            sub_label_main = sub_label.rstrip('%').strip()
            sub_label_percentage = sub_label.strip()

            # Regex to match values like "51 (43.14%)"
            value_match = re.match(r'^(.*?)\s*\((.*?)\)$', value)
            if value_match:
                main_value = value_match.group(1).strip()
                sub_value = value_match.group(2).strip()
                return [
                    (f"{main_label} {sub_label_main}", main_value),
                    (f"{main_label} {sub_label_percentage}", sub_value)
                ]
            else:
                # If value doesn't have '(', only main value
                return [
                    (f"{main_label} {sub_label_main}", value),
                    (f"{main_label} {sub_label_percentage}", '')
                ]
        else:
            # If sub_label does not end with '%', handle accordingly
            # For now, return as one key-value pair
            return [(f"{main_label} {sub_label}", value)]
    else:
        return [(label, value)]
