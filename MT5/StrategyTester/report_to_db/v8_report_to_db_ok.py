...
def split_specific_fields(label, value, fields_to_split):
    ...
    if label in fields_to_split:
        # Regex to match values like "51 (43.14%)" or "10.94% (109.40)"
        ...
        if value_match:
            ...
            if 'won' in label.lower():
                ...
                main_label = label.split('(')[0].strip() + ' won'
                sub_label = label.split('(')[0].strip() + ' won %'
                ...
                sub_label = label.split('(')[0].strip() + ' % of total'
