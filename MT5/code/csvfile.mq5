...
string csvFileName = "trading_scores.csv";
...
void WriteScoresToCSV(string type, double totalScore)
{
   ...
   string csvFilePath = StringConcatenate(TerminalInfoString(TERMINAL_DATA_PATH), "\\MQL5\\Files\\", csvFileName); 
   ...
   if(fileHandle != INVALID_HANDLE)
   {
      ...
      // **Important: Move file pointer to the end of the file**
      if(FileSeek(fileHandle, 0, SEEK_END) != 0) {
         Print("Error seeking to end of file: ", GetLastError());
         return; // Stop execution if seeking fails
      }

      // Append scores to the CSV file
      if(FileWriteString(fileHandle, scoreString) == 0) {
         Print("Error writing to CSV file: ", GetLastError());
   ...
   else 
   {
      ...
      Print("Error opening CSV file: ", GetLastError(), " | File path: ", csvFilePath);
   }
}
...
void WriteScoresToCSV(string type, double totalScore)
{
   ...
   if(fileHandle != INVALID_HANDLE)
   {
      string dateTimeString = TimeToString(TimeCurrent(), TIME_DATE) + ";" + TimeToString(TimeCurrent(), TIME_MINUTES) + ":" + TimeToString(TimeCurrent(), TIME_SECONDS);
      string scoreString = StringFormat("%s;%s;%.2f;%.2f;%.2f;%.2f;%.2f;%.2f;%.2f;%.2f;%.2f;%.2f;%.2f\n", 
                                      dateTimeString, type,
                                      CalculateMovingAverageScore(), CalculateMACDScore(),
                                      CalculateRSIScore(), CalculateStochasticScore(),
                                      CalculateBollingerBandsScore(), CalculateATRScore(),
      ...
      FileWrite(fileHandle, scoreString);
      FileClose(fileHandle);
   } 
   else {
      Print("Error writing to CSV file: ", GetLastError()); 
