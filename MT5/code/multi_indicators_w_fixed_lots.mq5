...
void OnTick()
{
    ...
    // Debug output
    Print("Total Score: ", totalScore, " | Open Positions: ", totalPositions);
    
    // Check Buy Condition
    if(totalScore >= BuyThreshold && totalPositions == 0)
    {
        Print("Attempting to open Buy position. Score: ", totalScore);
        if(trade.Buy(LotSize, _Symbol, 0, 0, 0, "Buy"))
        {
    ...
    {
        Print("Attempting to open Sell position. Score: ", totalScore);
        if(trade.Sell(LotSize, _Symbol, 0, 0, 0, "Sell"))
        {
