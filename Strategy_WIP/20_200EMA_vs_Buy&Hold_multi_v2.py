...
def main():
    ...
    if strategy_returns and buy_hold_returns:
        ...
        summary_df['Strategy'] = summary_df['Strategy'].apply(lambda x: f"{x:,.2f}")
        summary_df['Buy and Hold'] = summary_df['Buy and Hold'].apply(lambda x: f"{x:,.2f}")

        print("\nFinal Portfolio Values with Trade Counts:")
        ...
        print("\nSummary of Final Portfolio Values and Trade Counts:")
        print(summary_df.to_string(index=False))
