...
END_DATE = '2022-06-01'
MIN_SUCCESS_RATE = 0.60  # Target success rate
...
def run_backtest(params):
    ...
    cerebro.addstrategy(ImprovedBreakoutStrategy, params=params)
    results = cerebro.run()
    return results[0]

def main():
    success_rate = 0
    while success_rate < MIN_SUCCESS_RATE:
        # Generate random parameters
        params = {
            'resistance_period': np.random.randint(5, 20),
            'volume_multiplier': np.random.uniform(1.0, 1.5)
            # Add more parameters
        }
        results = run_backtest(params)
        success_rate = len(results.bull_runs) / len(results.breakouts) * 100 if results.breakouts else 0
