...
class ImprovedBreakoutStrategy(bt.Strategy):
    params = (
        ...
        ('bull_run_period', 10),  # Using bars for now, but consider time-based logic
        ('rsi_period', 14),
        ('rsi_threshold', 55),
        ('ma_period', 50),
        ('macd_fast', 12),
    ...
    def __init__(self):
        ...
        self.buy_price = None
        self.buy_date = None
        self.buy_bar_count = 0  # Counter for bars since buy
                    ...
                    if current_close > current_ma:
                        if current_macd > current_macd_signal:
                            if not self.position:
                                ...
                                self.buy_bar_count = 0  # Reset bar count on buy
                                self.breakouts.append((current_date, current_close))

        if self.position:
            self.buy_bar_count += 1  # Increment the bar counter
            ...
            elif self.buy_bar_count > self.p.bull_run_period:  # Using bar count
                self.close()
