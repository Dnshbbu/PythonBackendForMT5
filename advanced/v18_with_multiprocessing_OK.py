...
def worker(params, data):
    final_value, strategy = run_backtest(params, data)
    if final_value is not None and strategy is not None:
        strategy_return = (final_value - INITIAL_CASH) / INITIAL_CASH
        return (params, strategy_return, strategy)
    else:
...
def optimize_strategy(data):
    ...
    } for (resistance_period, volume_multiplier, bull_run_threshold, bull_run_period, stop_loss_atr_multiplier) in param_combinations]

    # --- 8. Parallel Optimization ---
    ...
    with open('iteration_results.csv', 'w', newline='') as csvfile:
        ...
        for i, result in enumerate(results):
            params, strategy_return, strategy = result
    ...
    # --- 10. Save Final Results and Analysis to a Separate File ---
    with open('final_results.txt', 'w') as f:
        ...
        print(f"\nStrategy identified {len(best_strategy.breakouts)} breakouts, "
        ...
        fig_hist.update_layout(title='Distribution of Returns from Optimization',
                              xaxis_title='Returns', yaxis_title='Frequency')
        # fig_hist.show() # Do not show the plot
