...
class ImprovedBreakoutStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            # Update trailing stop if current close is higher than previous high
            ...
            # Check for trailing stop (take profit)
...
def optimize_strategy(data):
    ...
    with open('final_results.txt', 'w') as f:
        ...
        # Re-run backtest with best parameters to get detailed trade logs
        ...
        if strategy is not None:
            ...
            fig.update_layout(
                title='Breakouts and Trailing Stop Exits (Optimized)',
                xaxis_title='Date',
                yaxis_title='Price',
                xaxis_rangeslider_visible=True
            )
            # Save the plot as an HTML file
            fig.write_html("breakouts_trailing_stops.html")
            print("\nVisualization saved as 'breakouts_trailing_stops.html'.")

            # Summary
            print(f"\nStrategy identified {len(strategy.breakouts)} breakouts, "
                  f"{len(strategy.stop_losses)} stop-loss exits, and "
                  f"{len(strategy.trailing_stops)} trailing stop exits.")
