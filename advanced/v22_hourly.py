...
END_DATE = '2023-01-15'  # Changed end date to January 31, 2019
INITIAL_CASH = 1000  # Initial investment in euros

# --- 3. Fetch Data ---
def fetch_data(ticker, start, end):
    """
    Fetches historical stock data from Yahoo Finance with hourly interval.
    """
    data = yf.download(ticker, start=start, end=end, interval='1h')  # Added interval='1h'
    if data.empty:
...
def optimize_strategy(data):
    ...
    } for (resistance_period, volume_multiplier, trailing_stop_atr_multipliers) in param_combinations]
