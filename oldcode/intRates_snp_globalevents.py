...
def get_global_events():
    ...
    events_df['Date'] = pd.to_datetime(events_df['Date'])  # Ensure date is in datetime format
    return events_df

# Plot interest rates and S&P 500 with dual y-axes and global events
def plot_data(interest_rate_df, snp_df, events_df):
    fig = go.Figure()
    ...
    # Add global events as annotations
    for _, row in events_df.iterrows():
        fig.add_annotation(
            x=row['Date'],
            y=interest_rate_df['Rate'].max(),  # Position annotation at the maximum interest rate value
            text=row['Event'],
            showarrow=True,
            arrowhead=1,
            ax=0,
    ...
    fig.update_layout(
        title="Interest Rate and S&P 500 with Major Global Events",
        xaxis_title="Date",
...
if __name__ == '__main__':
    ...
    # Get global events
    events_df = get_global_events()

    # Plot data
    plot_data(interest_rate_df, snp_df, events_df)
